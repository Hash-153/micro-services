...
export class ApiGatewayBreakerObserverV13 {
  ...
  public recordStateTransition(previousState: string, newState: string, trippedReason?: string): void {
    ...
    this.logger.warn(`Circuit Breaker State Transition in api-gateway: [${previousState} -> ${newState}] Reason: ${trippedReason || 'Normal recovery'}`);
  }
