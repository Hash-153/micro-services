...
export class OrderServiceEventHandlerV2 {
  ...
  public async handleEvent(event: EventMessageV2): Promise<{ isSuccess: boolean; shouldRetry: boolean; error?: string }> {
    this.logger.info(`Processing event ${event.eventId} (${event.eventType}) in order-service`);
    ...
    try {
      ...
      this.logger.error(`Event processing failed for ${event.eventId} in order-service:`, err);
  ...
  private async persistEventLog(event: EventMessageV2): Promise<void> {
    this.logger.info(`Persisted event audit log ${event.eventId} to order-service_event_journal`);
  }

  private async routeToDeadLetterQueue(event: EventMessageV2, reason: string): Promise<void> {
    this.logger.warn(`Routed exhausted event ${event.eventId} to DLQ in order-service: ${reason}`);
  }
}
