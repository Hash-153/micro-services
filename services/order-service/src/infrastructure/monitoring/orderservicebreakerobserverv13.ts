...
export class OrderServiceBreakerObserverV13 {
  ...
  public recordStateTransition(previousState: string, newState: string, trippedReason?: string): void {
    ...
    this.logger.warn(`Circuit Breaker State Transition in order-service: [${previousState} -> ${newState}] Reason: ${trippedReason || 'Normal recovery'}`);
  }
