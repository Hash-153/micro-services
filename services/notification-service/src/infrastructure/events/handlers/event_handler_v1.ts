...
export class NotificationServiceEventHandlerV1 {
  ...
  public async handleEvent(event: EventMessageV1): Promise<{ isSuccess: boolean; shouldRetry: boolean; error?: string }> {
    this.logger.info(`Processing event ${event.eventId} (${event.eventType}) in notification-service`);
    ...
    try {
      ...
      this.logger.error(`Event processing failed for ${event.eventId} in notification-service:`, err);
  ...
  private async persistEventLog(event: EventMessageV1): Promise<void> {
    this.logger.info(`Persisted event audit log ${event.eventId} to notification-service_event_journal`);
  }

  private async routeToDeadLetterQueue(event: EventMessageV1, reason: string): Promise<void> {
    this.logger.warn(`Routed exhausted event ${event.eventId} to DLQ in notification-service: ${reason}`);
  }
}
