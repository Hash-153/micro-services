...
export interface RepositoryEntityV2 {
  id: string;
  tenantId: string;
  entityCode: string;
  displayName: string;
...
export class NotificationServiceRepositoryV2 {
  private db: DatabaseClient;
  private logger: Logger;
  private readonly tableName: string = 'notification_table_v2';
  ...
  public async findById(id: string): Promise<RepositoryEntityV2 | null> {
    const sql = `SELECT * FROM "${this.tableName}" WHERE id = $1 AND is_deleted = FALSE;`;
    const rows = await this.db.query<RepositoryEntityV2>(sql, [id]);
    return rows.length > 0 ? rows[0] : null;
  }

  public async findByTenant(tenantId: string, limit: number = 50, offset: number = 0): Promise<RepositoryEntityV2[]> {
    const sql = `SELECT * FROM "${this.tableName}" WHERE tenant_id = $1 AND is_deleted = FALSE ORDER BY created_at DESC LIMIT $2 OFFSET $3;`;
    return await this.db.query<RepositoryEntityV2>(sql, [tenantId, limit, offset]);
  }

  public async create(entity: Omit<RepositoryEntityV2, 'createdAt' | 'updatedAt' | 'version'>): Promise<RepositoryEntityV2> {
    const sql = `
    ...
    const rows = await this.db.query<RepositoryEntityV2>(sql, params);
    return rows[0];
  }

  public async update(id: string, updates: Partial<RepositoryEntityV2>): Promise<RepositoryEntityV2 | null> {
    const sql = `
