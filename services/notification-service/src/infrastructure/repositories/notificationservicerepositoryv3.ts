...
export interface RepositoryEntityV3 {
  id: string;
  tenantId: string;
  entityCode: string;
  displayName: string;
...
export class NotificationServiceRepositoryV3 {
  private db: DatabaseClient;
  private logger: Logger;
  private readonly tableName: string = 'notification_table_v3';
  ...
  public async findById(id: string): Promise<RepositoryEntityV3 | null> {
    const sql = `SELECT * FROM "${this.tableName}" WHERE id = $1 AND is_deleted = FALSE;`;
    const rows = await this.db.query<RepositoryEntityV3>(sql, [id]);
    return rows.length > 0 ? rows[0] : null;
  }

  public async findByTenant(tenantId: string, limit: number = 50, offset: number = 0): Promise<RepositoryEntityV3[]> {
    const sql = `SELECT * FROM "${this.tableName}" WHERE tenant_id = $1 AND is_deleted = FALSE ORDER BY created_at DESC LIMIT $2 OFFSET $3;`;
    return await this.db.query<RepositoryEntityV3>(sql, [tenantId, limit, offset]);
  }

  public async create(entity: Omit<RepositoryEntityV3, 'createdAt' | 'updatedAt' | 'version'>): Promise<RepositoryEntityV3> {
    const sql = `
    ...
    const rows = await this.db.query<RepositoryEntityV3>(sql, params);
    return rows[0];
  }

  public async update(id: string, updates: Partial<RepositoryEntityV3>): Promise<RepositoryEntityV3 | null> {
    const sql = `
