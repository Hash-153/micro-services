export interface SubsystemHealthStatusV8 {
...
export interface OverallHealthReportV8 {
  serviceName: 'notification-service';
  status: 'HEALTHY' | 'DEGRADED' | 'UNHEALTHY';
  uptimeSeconds: number;
  subsystems: SubsystemHealthStatusV8[];
  checkedAt: Date;
}

export class NotificationServiceHealthDiagnosticV8 {
  private startTime: number = Date.now();

  public async runFullDiagnostics(): Promise<OverallHealthReportV8> {
    const subsystems: SubsystemHealthStatusV8[] = [
    ...
    let status: OverallHealthReportV8['status'] = 'HEALTHY';
