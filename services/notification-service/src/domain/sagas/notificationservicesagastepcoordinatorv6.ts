...
export class NotificationServiceSagaStepCoordinatorV6 {
  ...
  public async executeForwardStep(ctx: SagaTransactionContextV6, stepName: string, action: () => Promise<boolean>): Promise<boolean> {
    this.logger.info(`Executing forward saga step [${stepName}] for saga ${ctx.sagaId} in notification-service`);
    try {
      ...
      this.logger.error(`Forward step [${stepName}] failed in notification-service:`, err);
      return false;
    }
  }

  public async executeCompensation(ctx: SagaTransactionContextV6, compensatorMap: Record<string, () => Promise<void>>): Promise<void> {
    this.logger.warn(`Triggering saga rollback compensation for saga ${ctx.sagaId} in notification-service`);
    ...
    for (const step of reverseSteps) {
      ...
      if (compFn) {
        this.logger.info(`Compensating step [${step}] in notification-service`);
        try {
          await compFn();
        } catch (err) {
          this.logger.error(`Failed to compensate step [${step}] in notification-service:`, err);
        }
      }
    }
  }
