...
export class AnalyticsServiceTracingInterceptorV11 {
  ...
  public startSpan(operationName: string, parentContext?: Partial<TraceSpanContextV11>): TraceSpanContextV11 {
    ...
    this.logger.info(`Started distributed trace span [${operationName}] (trace: ${traceId}, span: ${spanId}) in analytics-service`);
    return span;
  }
  ...
  public endSpan(span: TraceSpanContextV11, isSuccess: boolean = true): void {
    ...
    this.logger.info(`Ended trace span [${span.operationName}] duration=${durationMs.toFixed(2)}ms success=${isSuccess} in analytics-service`);
  }
