...
export class FulfillmentServiceEventHandlerV1 {
  ...
  public async handleEvent(event: EventMessageV1): Promise<{ isSuccess: boolean; shouldRetry: boolean; error?: string }> {
    this.logger.info(`Processing event ${event.eventId} (${event.eventType}) in fulfillment-service`);
    ...
    try {
      ...
      this.logger.error(`Event processing failed for ${event.eventId} in fulfillment-service:`, err);
  ...
  private async persistEventLog(event: EventMessageV1): Promise<void> {
    this.logger.info(`Persisted event audit log ${event.eventId} to fulfillment-service_event_journal`);
  }

  private async routeToDeadLetterQueue(event: EventMessageV1, reason: string): Promise<void> {
    this.logger.warn(`Routed exhausted event ${event.eventId} to DLQ in fulfillment-service: ${reason}`);
  }
}
