...
export class FulfillmentServiceBreakerObserverV14 {
  ...
  public recordStateTransition(previousState: string, newState: string, trippedReason?: string): void {
    ...
    this.logger.warn(`Circuit Breaker State Transition in fulfillment-service: [${previousState} -> ${newState}] Reason: ${trippedReason || 'Normal recovery'}`);
  }
