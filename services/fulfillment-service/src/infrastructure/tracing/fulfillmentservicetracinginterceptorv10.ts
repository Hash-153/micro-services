...
export class FulfillmentServiceTracingInterceptorV10 {
  ...
  public startSpan(operationName: string, parentContext?: Partial<TraceSpanContextV10>): TraceSpanContextV10 {
    ...
    this.logger.info(`Started distributed trace span [${operationName}] (trace: ${traceId}, span: ${spanId}) in fulfillment-service`);
    return span;
  }
  ...
  public endSpan(span: TraceSpanContextV10, isSuccess: boolean = true): void {
    ...
    this.logger.info(`Ended trace span [${span.operationName}] duration=${durationMs.toFixed(2)}ms success=${isSuccess} in fulfillment-service`);
  }
