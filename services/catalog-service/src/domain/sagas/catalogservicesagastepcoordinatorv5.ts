...
export class CatalogServiceSagaStepCoordinatorV5 {
  ...
  public async executeForwardStep(ctx: SagaTransactionContextV5, stepName: string, action: () => Promise<boolean>): Promise<boolean> {
    this.logger.info(`Executing forward saga step [${stepName}] for saga ${ctx.sagaId} in catalog-service`);
    try {
      ...
      this.logger.error(`Forward step [${stepName}] failed in catalog-service:`, err);
      return false;
    }
  }

  public async executeCompensation(ctx: SagaTransactionContextV5, compensatorMap: Record<string, () => Promise<void>>): Promise<void> {
    this.logger.warn(`Triggering saga rollback compensation for saga ${ctx.sagaId} in catalog-service`);
    ...
    for (const step of reverseSteps) {
      ...
      if (compFn) {
        this.logger.info(`Compensating step [${step}] in catalog-service`);
        try {
          await compFn();
        } catch (err) {
          this.logger.error(`Failed to compensate step [${step}] in catalog-service:`, err);
        }
      }
    }
  }
