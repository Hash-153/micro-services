export interface NvmeJbodSpecConfig {
  specId: string;
  categorySlug: 'nvme-jbod';
  categoryName: 'NVMe-oF Just a Bunch of Flash (JBOF)';
...
export const NVME_JBOD_DEFAULT_SPEC: NvmeJbodSpecConfig = {
  specId: 'spec_nvme-jbod_v1',
  categorySlug: 'nvme-jbod',
  categoryName: 'NVMe-oF Just a Bunch of Flash (JBOF)',
  hardwareDomain: 'Storage',
...
export class NvmeJbodSpecValidator {
  public static validate(config: NvmeJbodSpecConfig): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  public static getDeratingCurve(ambientTempCelsius: number, config: NvmeJbodSpecConfig = NVME_JBOD_DEFAULT_SPEC): number {
