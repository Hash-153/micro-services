export interface SatelliteModemSpecConfig {
  specId: string;
  categorySlug: 'satellite-modem';
  categoryName: 'High-Throughput LEO Satellite Ground Modems';
...
export const SATELLITE_MODEM_DEFAULT_SPEC: SatelliteModemSpecConfig = {
  specId: 'spec_satellite-modem_v1',
  categorySlug: 'satellite-modem',
  categoryName: 'High-Throughput LEO Satellite Ground Modems',
  hardwareDomain: 'Networking',
...
export class SatelliteModemSpecValidator {
  public static validate(config: SatelliteModemSpecConfig): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  public static getDeratingCurve(ambientTempCelsius: number, config: SatelliteModemSpecConfig = SATELLITE_MODEM_DEFAULT_SPEC): number {
