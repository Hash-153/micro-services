export interface RuggedServerSpecConfig {
  specId: string;
  categorySlug: 'rugged-server';
  categoryName: 'MIL-STD-810H Ruggedized Edge Tactical Servers';
...
export const RUGGED_SERVER_DEFAULT_SPEC: RuggedServerSpecConfig = {
  specId: 'spec_rugged-server_v1',
  categorySlug: 'rugged-server',
  categoryName: 'MIL-STD-810H Ruggedized Edge Tactical Servers',
  hardwareDomain: 'Compute',
...
export class RuggedServerSpecValidator {
  public static validate(config: RuggedServerSpecConfig): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  public static getDeratingCurve(ambientTempCelsius: number, config: RuggedServerSpecConfig = RUGGED_SERVER_DEFAULT_SPEC): number {
