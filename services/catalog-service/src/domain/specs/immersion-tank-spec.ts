export interface ImmersionTankSpecConfig {
  specId: string;
  categorySlug: 'immersion-tank';
  categoryName: 'Two-Phase Liquid Immersion Tanks';
...
export const IMMERSION_TANK_DEFAULT_SPEC: ImmersionTankSpecConfig = {
  specId: 'spec_immersion-tank_v1',
  categorySlug: 'immersion-tank',
  categoryName: 'Two-Phase Liquid Immersion Tanks',
  hardwareDomain: 'Infrastructure',
...
export class ImmersionTankSpecValidator {
  public static validate(config: ImmersionTankSpecConfig): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  public static getDeratingCurve(ambientTempCelsius: number, config: ImmersionTankSpecConfig = IMMERSION_TANK_DEFAULT_SPEC): number {
