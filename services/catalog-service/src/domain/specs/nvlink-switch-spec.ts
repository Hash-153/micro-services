export interface NvlinkSwitchSpecConfig {
  specId: string;
  categorySlug: 'nvlink-switch';
  categoryName: 'High-Speed NVLink 4.0 Switching Fabric Units';
...
export const NVLINK_SWITCH_DEFAULT_SPEC: NvlinkSwitchSpecConfig = {
  specId: 'spec_nvlink-switch_v1',
  categorySlug: 'nvlink-switch',
  categoryName: 'High-Speed NVLink 4.0 Switching Fabric Units',
  hardwareDomain: 'Compute',
...
export class NvlinkSwitchSpecValidator {
  public static validate(config: NvlinkSwitchSpecConfig): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  public static getDeratingCurve(ambientTempCelsius: number, config: NvlinkSwitchSpecConfig = NVLINK_SWITCH_DEFAULT_SPEC): number {
