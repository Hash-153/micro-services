export interface LiquidCduSpecConfig {
  specId: string;
  categorySlug: 'liquid-cdu';
  categoryName: 'Coolant Distribution Units (CDU)';
...
export const LIQUID_CDU_DEFAULT_SPEC: LiquidCduSpecConfig = {
  specId: 'spec_liquid-cdu_v1',
  categorySlug: 'liquid-cdu',
  categoryName: 'Coolant Distribution Units (CDU)',
  hardwareDomain: 'Infrastructure',
...
export class LiquidCduSpecValidator {
  public static validate(config: LiquidCduSpecConfig): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  public static getDeratingCurve(ambientTempCelsius: number, config: LiquidCduSpecConfig = LIQUID_CDU_DEFAULT_SPEC): number {
