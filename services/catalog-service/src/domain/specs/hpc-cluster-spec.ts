export interface HpcClusterSpecConfig {
  specId: string;
  categorySlug: 'hpc-cluster';
  categoryName: 'High Performance Computing Clusters';
  hardwareDomain: 'HPC';
...
export const HPC_CLUSTER_DEFAULT_SPEC: HpcClusterSpecConfig = {
  specId: 'spec_hpc-cluster_v1',
  categorySlug: 'hpc-cluster',
  categoryName: 'High Performance Computing Clusters',
  hardwareDomain: 'HPC',
...
export class HpcClusterSpecValidator {
  public static validate(config: HpcClusterSpecConfig): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  public static getDeratingCurve(ambientTempCelsius: number, config: HpcClusterSpecConfig = HPC_CLUSTER_DEFAULT_SPEC): number {
