...
export class PaymentServiceTracingInterceptorV12 {
  ...
  public startSpan(operationName: string, parentContext?: Partial<TraceSpanContextV12>): TraceSpanContextV12 {
    ...
    this.logger.info(`Started distributed trace span [${operationName}] (trace: ${traceId}, span: ${spanId}) in payment-service`);
    return span;
  }
  ...
  public endSpan(span: TraceSpanContextV12, isSuccess: boolean = true): void {
    ...
    this.logger.info(`Ended trace span [${span.operationName}] duration=${durationMs.toFixed(2)}ms success=${isSuccess} in payment-service`);
  }
