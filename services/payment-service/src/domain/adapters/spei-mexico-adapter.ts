...
export interface SpeiMexicoAdapterConfig {
  adapterId: string;
  schemeName: 'SPEI Interbank Electronic Payment System';
  primaryCurrency: 'MXN';
...
export const SPEI_MEXICO_CONFIG: SpeiMexicoAdapterConfig = {
  adapterId: 'pm_spei_mexico_v1',
  schemeName: 'SPEI Interbank Electronic Payment System',
  primaryCurrency: 'MXN',
  jurisdictionCountry: 'MX',
...
export class SpeiMexicoAdapter {
  private config: SpeiMexicoAdapterConfig;

  constructor(config: SpeiMexicoAdapterConfig = SPEI_MEXICO_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_spei_mexico_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
