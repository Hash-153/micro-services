...
export interface PayuLatamAdapterConfig {
  adapterId: string;
  schemeName: 'PayU Latin America Aggregator';
  primaryCurrency: 'USD';
  jurisdictionCountry: 'LATAM';
...
export const PAYU_LATAM_CONFIG: PayuLatamAdapterConfig = {
  adapterId: 'pm_payu_latam_v1',
  schemeName: 'PayU Latin America Aggregator',
  primaryCurrency: 'USD',
  jurisdictionCountry: 'LATAM',
...
export class PayuLatamAdapter {
  private config: PayuLatamAdapterConfig;

  constructor(config: PayuLatamAdapterConfig = PAYU_LATAM_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_payu_latam_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
