...
export interface IdealNetherlandsAdapterConfig {
  adapterId: string;
  schemeName: 'iDEAL Online Banking';
  primaryCurrency: 'EUR';
...
export const IDEAL_NETHERLANDS_CONFIG: IdealNetherlandsAdapterConfig = {
  adapterId: 'pm_ideal_netherlands_v1',
  schemeName: 'iDEAL Online Banking',
  primaryCurrency: 'EUR',
  jurisdictionCountry: 'NL',
...
export class IdealNetherlandsAdapter {
  private config: IdealNetherlandsAdapterConfig;

  constructor(config: IdealNetherlandsAdapterConfig = IDEAL_NETHERLANDS_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_ideal_netherlands_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
