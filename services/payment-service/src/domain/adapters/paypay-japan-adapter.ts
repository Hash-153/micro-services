...
export interface PaypayJapanAdapterConfig {
  adapterId: string;
  schemeName: 'PayPay QR Code Merchant Payments';
  primaryCurrency: 'JPY';
...
export const PAYPAY_JAPAN_CONFIG: PaypayJapanAdapterConfig = {
  adapterId: 'pm_paypay_japan_v1',
  schemeName: 'PayPay QR Code Merchant Payments',
  primaryCurrency: 'JPY',
  jurisdictionCountry: 'JP',
...
export class PaypayJapanAdapter {
  private config: PaypayJapanAdapterConfig;

  constructor(config: PaypayJapanAdapterConfig = PAYPAY_JAPAN_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_paypay_japan_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
