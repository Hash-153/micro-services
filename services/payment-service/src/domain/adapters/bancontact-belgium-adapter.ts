...
export interface BancontactBelgiumAdapterConfig {
  adapterId: string;
  schemeName: 'Bancontact Debit Cards';
  primaryCurrency: 'EUR';
  jurisdictionCountry: 'BE';
...
export const BANCONTACT_BELGIUM_CONFIG: BancontactBelgiumAdapterConfig = {
  adapterId: 'pm_bancontact_belgium_v1',
  schemeName: 'Bancontact Debit Cards',
  primaryCurrency: 'EUR',
  jurisdictionCountry: 'BE',
...
export class BancontactBelgiumAdapter {
  private config: BancontactBelgiumAdapterConfig;

  constructor(config: BancontactBelgiumAdapterConfig = BANCONTACT_BELGIUM_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_bancontact_belgium_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
