...
export interface BacsDirectDebitAdapterConfig {
  adapterId: string;
  schemeName: 'BACS Direct Debit';
  primaryCurrency: 'GBP';
...
export const BACS_DIRECT_DEBIT_CONFIG: BacsDirectDebitAdapterConfig = {
  adapterId: 'pm_bacs_direct_debit_v1',
  schemeName: 'BACS Direct Debit',
  primaryCurrency: 'GBP',
  jurisdictionCountry: 'GB',
...
export class BacsDirectDebitAdapter {
  private config: BacsDirectDebitAdapterConfig;

  constructor(config: BacsDirectDebitAdapterConfig = BACS_DIRECT_DEBIT_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_bacs_direct_debit_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
