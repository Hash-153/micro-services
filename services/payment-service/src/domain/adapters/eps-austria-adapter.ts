...
export interface EpsAustriaAdapterConfig {
  adapterId: string;
  schemeName: 'EPS Online-Überweisung';
  primaryCurrency: 'EUR';
  jurisdictionCountry: 'AT';
...
export const EPS_AUSTRIA_CONFIG: EpsAustriaAdapterConfig = {
  adapterId: 'pm_eps_austria_v1',
  schemeName: 'EPS Online-Überweisung',
  primaryCurrency: 'EUR',
  jurisdictionCountry: 'AT',
...
export class EpsAustriaAdapter {
  private config: EpsAustriaAdapterConfig;

  constructor(config: EpsAustriaAdapterConfig = EPS_AUSTRIA_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_eps_austria_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
