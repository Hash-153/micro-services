...
export interface FasterPaymentsAdapterConfig {
  adapterId: string;
  schemeName: 'UK Faster Payments Service (FPS)';
  primaryCurrency: 'GBP';
...
export const FASTER_PAYMENTS_CONFIG: FasterPaymentsAdapterConfig = {
  adapterId: 'pm_faster_payments_v1',
  schemeName: 'UK Faster Payments Service (FPS)',
  primaryCurrency: 'GBP',
  jurisdictionCountry: 'GB',
...
export class FasterPaymentsAdapter {
  private config: FasterPaymentsAdapterConfig;

  constructor(config: FasterPaymentsAdapterConfig = FASTER_PAYMENTS_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_faster_payments_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
