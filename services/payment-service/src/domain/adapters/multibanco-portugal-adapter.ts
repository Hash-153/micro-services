...
export interface MultibancoPortugalAdapterConfig {
  adapterId: string;
  schemeName: 'Multibanco Reference Payments';
  primaryCurrency: 'EUR';
  jurisdictionCountry: 'PT';
...
export const MULTIBANCO_PORTUGAL_CONFIG: MultibancoPortugalAdapterConfig = {
  adapterId: 'pm_multibanco_portugal_v1',
  schemeName: 'Multibanco Reference Payments',
  primaryCurrency: 'EUR',
  jurisdictionCountry: 'PT',
...
export class MultibancoPortugalAdapter {
  private config: MultibancoPortugalAdapterConfig;

  constructor(config: MultibancoPortugalAdapterConfig = MULTIBANCO_PORTUGAL_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_multibanco_portugal_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
