...
export interface BlikPolandAdapterConfig {
  adapterId: string;
  schemeName: 'BLIK Mobile 6-Digit Code Payments';
  primaryCurrency: 'PLN';
...
export const BLIK_POLAND_CONFIG: BlikPolandAdapterConfig = {
  adapterId: 'pm_blik_poland_v1',
  schemeName: 'BLIK Mobile 6-Digit Code Payments',
  primaryCurrency: 'PLN',
  jurisdictionCountry: 'PL',
...
export class BlikPolandAdapter {
  private config: BlikPolandAdapterConfig;

  constructor(config: BlikPolandAdapterConfig = BLIK_POLAND_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_blik_poland_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
