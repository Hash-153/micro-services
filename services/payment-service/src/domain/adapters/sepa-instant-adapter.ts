...
export interface SepaInstantAdapterConfig {
  adapterId: string;
  schemeName: 'SEPA Instant Credit Transfer (SCT Inst)';
  primaryCurrency: 'EUR';
...
export const SEPA_INSTANT_CONFIG: SepaInstantAdapterConfig = {
  adapterId: 'pm_sepa_instant_v1',
  schemeName: 'SEPA Instant Credit Transfer (SCT Inst)',
  primaryCurrency: 'EUR',
  jurisdictionCountry: 'EU',
...
export class SepaInstantAdapter {
  private config: SepaInstantAdapterConfig;

  constructor(config: SepaInstantAdapterConfig = SEPA_INSTANT_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_sepa_instant_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
