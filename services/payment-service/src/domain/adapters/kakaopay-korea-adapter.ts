...
export interface KakaopayKoreaAdapterConfig {
  adapterId: string;
  schemeName: 'KakaoPay Mobile Wallet';
  primaryCurrency: 'KRW';
...
export const KAKAOPAY_KOREA_CONFIG: KakaopayKoreaAdapterConfig = {
  adapterId: 'pm_kakaopay_korea_v1',
  schemeName: 'KakaoPay Mobile Wallet',
  primaryCurrency: 'KRW',
  jurisdictionCountry: 'KR',
...
export class KakaopayKoreaAdapter {
  private config: KakaopayKoreaAdapterConfig;

  constructor(config: KakaopayKoreaAdapterConfig = KAKAOPAY_KOREA_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_kakaopay_korea_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
