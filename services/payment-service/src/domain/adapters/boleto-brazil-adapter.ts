...
export interface BoletoBrazilAdapterConfig {
  adapterId: string;
  schemeName: 'Boleto Bancário Barcode Invoice';
  primaryCurrency: 'BRL';
...
export const BOLETO_BRAZIL_CONFIG: BoletoBrazilAdapterConfig = {
  adapterId: 'pm_boleto_brazil_v1',
  schemeName: 'Boleto Bancário Barcode Invoice',
  primaryCurrency: 'BRL',
  jurisdictionCountry: 'BR',
...
export class BoletoBrazilAdapter {
  private config: BoletoBrazilAdapterConfig;

  constructor(config: BoletoBrazilAdapterConfig = BOLETO_BRAZIL_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_boleto_brazil_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
