...
export interface OxxoMexicoAdapterConfig {
  adapterId: string;
  schemeName: 'OXXO Cash Voucher Clearing';
  primaryCurrency: 'MXN';
...
export const OXXO_MEXICO_CONFIG: OxxoMexicoAdapterConfig = {
  adapterId: 'pm_oxxo_mexico_v1',
  schemeName: 'OXXO Cash Voucher Clearing',
  primaryCurrency: 'MXN',
  jurisdictionCountry: 'MX',
...
export class OxxoMexicoAdapter {
  private config: OxxoMexicoAdapterConfig;

  constructor(config: OxxoMexicoAdapterConfig = OXXO_MEXICO_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_oxxo_mexico_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
