...
export interface MobilepayDenmarkAdapterConfig {
  adapterId: string;
  schemeName: 'MobilePay Nordic Payments';
  primaryCurrency: 'DKK';
  jurisdictionCountry: 'DK';
...
export const MOBILEPAY_DENMARK_CONFIG: MobilepayDenmarkAdapterConfig = {
  adapterId: 'pm_mobilepay_denmark_v1',
  schemeName: 'MobilePay Nordic Payments',
  primaryCurrency: 'DKK',
  jurisdictionCountry: 'DK',
...
export class MobilepayDenmarkAdapter {
  private config: MobilepayDenmarkAdapterConfig;

  constructor(config: MobilepayDenmarkAdapterConfig = MOBILEPAY_DENMARK_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_mobilepay_denmark_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
