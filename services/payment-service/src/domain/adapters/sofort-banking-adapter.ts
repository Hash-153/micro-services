...
export interface SofortBankingAdapterConfig {
  adapterId: string;
  schemeName: 'Sofort Überweisung / Klarna Pay Now';
  primaryCurrency: 'EUR';
...
export const SOFORT_BANKING_CONFIG: SofortBankingAdapterConfig = {
  adapterId: 'pm_sofort_banking_v1',
  schemeName: 'Sofort Überweisung / Klarna Pay Now',
  primaryCurrency: 'EUR',
  jurisdictionCountry: 'DE',
...
export class SofortBankingAdapter {
  private config: SofortBankingAdapterConfig;

  constructor(config: SofortBankingAdapterConfig = SOFORT_BANKING_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_sofort_banking_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
