...
export interface SepaCreditTransferAdapterConfig {
  adapterId: string;
  schemeName: 'SEPA Credit Transfer (SCT)';
  primaryCurrency: 'EUR';
...
export const SEPA_CREDIT_TRANSFER_CONFIG: SepaCreditTransferAdapterConfig = {
  adapterId: 'pm_sepa_credit_transfer_v1',
  schemeName: 'SEPA Credit Transfer (SCT)',
  primaryCurrency: 'EUR',
  jurisdictionCountry: 'EU',
...
export class SepaCreditTransferAdapter {
  private config: SepaCreditTransferAdapterConfig;

  constructor(config: SepaCreditTransferAdapterConfig = SEPA_CREDIT_TRANSFER_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_sepa_credit_transfer_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
