...
export interface GiropayGermanyAdapterConfig {
  adapterId: string;
  schemeName: 'Giropay Online Banking';
  primaryCurrency: 'EUR';
...
export const GIROPAY_GERMANY_CONFIG: GiropayGermanyAdapterConfig = {
  adapterId: 'pm_giropay_germany_v1',
  schemeName: 'Giropay Online Banking',
  primaryCurrency: 'EUR',
  jurisdictionCountry: 'DE',
...
export class GiropayGermanyAdapter {
  private config: GiropayGermanyAdapterConfig;

  constructor(config: GiropayGermanyAdapterConfig = GIROPAY_GERMANY_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_giropay_germany_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
