...
export interface VippsNorwayAdapterConfig {
  adapterId: string;
  schemeName: 'Vipps Mobile Payments';
  primaryCurrency: 'NOK';
  jurisdictionCountry: 'NO';
...
export const VIPPS_NORWAY_CONFIG: VippsNorwayAdapterConfig = {
  adapterId: 'pm_vipps_norway_v1',
  schemeName: 'Vipps Mobile Payments',
  primaryCurrency: 'NOK',
  jurisdictionCountry: 'NO',
...
export class VippsNorwayAdapter {
  private config: VippsNorwayAdapterConfig;

  constructor(config: VippsNorwayAdapterConfig = VIPPS_NORWAY_CONFIG) {
    this.config = config;
  }
  ...
  public async initiatePayment(orderId: string, amountCents: number, customerEmail: string): Promise<{ transactionId: string; status: 'PENDING' | 'AUTHORIZED' | 'REQUIRES_REDIRECT'; redirectUrl?: string; checkoutToken?: string }> {
    const txnId = `txn_vipps_norway_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
