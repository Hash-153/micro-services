...
export class AuthServiceBreakerObserverV14 {
  ...
  public recordStateTransition(previousState: string, newState: string, trippedReason?: string): void {
    ...
    this.logger.warn(`Circuit Breaker State Transition in auth-service: [${previousState} -> ${newState}] Reason: ${trippedReason || 'Normal recovery'}`);
  }
