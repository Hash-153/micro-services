...
export class AuthServiceEventHandlerV3 {
  ...
  public async handleEvent(event: EventMessageV3): Promise<{ isSuccess: boolean; shouldRetry: boolean; error?: string }> {
    this.logger.info(`Processing event ${event.eventId} (${event.eventType}) in auth-service`);
    ...
    try {
      ...
      this.logger.error(`Event processing failed for ${event.eventId} in auth-service:`, err);
  ...
  private async persistEventLog(event: EventMessageV3): Promise<void> {
    this.logger.info(`Persisted event audit log ${event.eventId} to auth-service_event_journal`);
  }

  private async routeToDeadLetterQueue(event: EventMessageV3, reason: string): Promise<void> {
    this.logger.warn(`Routed exhausted event ${event.eventId} to DLQ in auth-service: ${reason}`);
  }
}
