...
export class AuthServiceSagaStepCoordinatorV4 {
  ...
  public async executeForwardStep(ctx: SagaTransactionContextV4, stepName: string, action: () => Promise<boolean>): Promise<boolean> {
    this.logger.info(`Executing forward saga step [${stepName}] for saga ${ctx.sagaId} in auth-service`);
    try {
      ...
      this.logger.error(`Forward step [${stepName}] failed in auth-service:`, err);
      return false;
    }
  }

  public async executeCompensation(ctx: SagaTransactionContextV4, compensatorMap: Record<string, () => Promise<void>>): Promise<void> {
    this.logger.warn(`Triggering saga rollback compensation for saga ${ctx.sagaId} in auth-service`);
    ...
    for (const step of reverseSteps) {
      ...
      if (compFn) {
        this.logger.info(`Compensating step [${step}] in auth-service`);
        try {
          await compFn();
        } catch (err) {
          this.logger.error(`Failed to compensate step [${step}] in auth-service:`, err);
        }
      }
    }
  }
