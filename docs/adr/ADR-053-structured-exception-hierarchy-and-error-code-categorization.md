# ADR-053: Structured Exception Hierarchy and Error Code Categorization
...
## Decision
We formally adopt **Structured Exception Hierarchy and Error Code Categorization**.
Standardize application exceptions on `AppError` base class with deterministic machine-readable `ErrorCode` enums.
