# ADR-148: API Gateway Response Header Hardening and XSS Protection
...
## Decision
We formally adopt **API Gateway Response Header Hardening and XSS Protection**.
Enforce `X-XSS-Protection: 1; mode=block` and `Referrer-Policy: strict-origin-when-cross-origin` on all responses.
