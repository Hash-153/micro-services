# ADR-084: Microservice Service Discovery using Kubernetes Native CoreDNS
...
## Decision
We formally adopt **Microservice Service Discovery using Kubernetes Native CoreDNS**.
Resolve internal microservice endpoints via cluster DNS (`http://service-name:port`).
