# ADR-111: Protobuf Backward Compatibility Linters in CI/CD
...
## Decision
We formally adopt **Protobuf Backward Compatibility Linters in CI/CD**.
Integrate `buf lint` and `buf breaking` into GitHub Actions pipelines to block breaking schema changes.
