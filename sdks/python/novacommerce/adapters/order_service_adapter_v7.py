...
class OrderServiceAsyncAdapterV7Config(BaseModel):
...
class OrderServiceAsyncAdapterV7:
    """High-performance async adapter for order-service version 7"""
    def __init__(self, client: httpx.AsyncClient, base_url: str, config: Optional[OrderServiceAsyncAdapterV7Config] = None):
        self.client = client
        self.base_url = base_url.rstrip('/') + "/api/v7/order"
        self.config = config or OrderServiceAsyncAdapterV7Config()
