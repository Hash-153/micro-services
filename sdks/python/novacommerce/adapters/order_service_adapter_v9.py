...
class OrderServiceAsyncAdapterV9Config(BaseModel):
...
class OrderServiceAsyncAdapterV9:
    """High-performance async adapter for order-service version 9"""
    def __init__(self, client: httpx.AsyncClient, base_url: str, config: Optional[OrderServiceAsyncAdapterV9Config] = None):
        self.client = client
        self.base_url = base_url.rstrip('/') + "/api/v9/order"
        self.config = config or OrderServiceAsyncAdapterV9Config()
