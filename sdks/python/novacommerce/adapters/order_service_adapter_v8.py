...
class OrderServiceAsyncAdapterV8Config(BaseModel):
...
class OrderServiceAsyncAdapterV8:
    """High-performance async adapter for order-service version 8"""
    def __init__(self, client: httpx.AsyncClient, base_url: str, config: Optional[OrderServiceAsyncAdapterV8Config] = None):
        self.client = client
        self.base_url = base_url.rstrip('/') + "/api/v8/order"
        self.config = config or OrderServiceAsyncAdapterV8Config()
