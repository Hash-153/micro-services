...
export interface AuditClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class AuditClient {
  ...
  private readonly basePath: string = '/api/v1/audits';
  ...
  public async list<T = any>(options?: AuditClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
