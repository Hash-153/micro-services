...
export interface WarehouseClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class WarehouseClient {
  ...
  private readonly basePath: string = '/api/v1/warehouses';
  ...
  public async list<T = any>(options?: WarehouseClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
