...
export interface RmaClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class RmaClient {
  private http: NovaCommerceHttpClient;
  private readonly basePath: string = '/api/v1/rma';
  ...
  public async list<T = any>(options?: RmaClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
