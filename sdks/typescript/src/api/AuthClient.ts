...
export interface AuthClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class AuthClient {
  ...
  private readonly basePath: string = '/api/v1/auth';
  ...
  public async list<T = any>(options?: AuthClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
