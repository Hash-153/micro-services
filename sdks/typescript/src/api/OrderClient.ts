...
export interface OrderClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class OrderClient {
  ...
  private readonly basePath: string = '/api/v1/orders';
  ...
  public async list<T = any>(options?: OrderClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
