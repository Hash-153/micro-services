...
export interface CarrierClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class CarrierClient {
  ...
  private readonly basePath: string = '/api/v1/carriers';
  ...
  public async list<T = any>(options?: CarrierClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
