...
export interface UserClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class UserClient {
  ...
  private readonly basePath: string = '/api/v1/users';
  ...
  public async list<T = any>(options?: UserClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
