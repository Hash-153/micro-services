...
export interface LedgerClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class LedgerClient {
  ...
  private readonly basePath: string = '/api/v1/ledger';
  ...
  public async list<T = any>(options?: LedgerClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
