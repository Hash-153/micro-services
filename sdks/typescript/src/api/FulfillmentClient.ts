...
export interface FulfillmentClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class FulfillmentClient {
  ...
  private readonly basePath: string = '/api/v1/fulfillment';
  ...
  public async list<T = any>(options?: FulfillmentClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
