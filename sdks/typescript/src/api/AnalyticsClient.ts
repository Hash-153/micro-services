...
export interface AnalyticsClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class AnalyticsClient {
  ...
  private readonly basePath: string = '/api/v1/analytics';
  ...
  public async list<T = any>(options?: AnalyticsClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
