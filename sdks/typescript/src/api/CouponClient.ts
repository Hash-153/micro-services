...
export interface CouponClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class CouponClient {
  ...
  private readonly basePath: string = '/api/v1/coupons';
  ...
  public async list<T = any>(options?: CouponClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
