...
export interface RecommendationClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class RecommendationClient {
  ...
  private readonly basePath: string = '/api/v1/recommendations';
  ...
  public async list<T = any>(options?: RecommendationClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
