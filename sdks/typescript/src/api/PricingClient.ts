...
export interface PricingClientFilterOptions {
  page?: number;
  limit?: number;
  search?: string;
  sortBy?: string;
...
export class PricingClient {
  ...
  private readonly basePath: string = '/api/v1/pricing';
  ...
  public async list<T = any>(options?: PricingClientFilterOptions): Promise<{ items: T[]; total: number; page: number; limit: number }> {
